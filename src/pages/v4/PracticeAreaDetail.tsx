      ...
      <div className="v4-theme min-h-screen">
        ...
        <main className="py-24">
          <div className="mono-container text-center">
            ...
            <Button asChild style={{ backgroundColor: "#002366", color: "#ffffff" }}>
              <Link to="/v4/practice-areas">View All Practice Areas</Link>
            </Button>
          </div>
        </main>
        <Footer />
          ...
          <div className="mono-container">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {data.services.map((service: string, idx: number) => (
                      <div key={idx} className="flex items-start gap-3">
                        <CheckCircle2 className="h-5 w-5 mt-0.5 flex-shrink-0 text-foreground" />
                    ...
                    {data.examples.map((example: string, idx: number) => (
                      <Card key={idx} className="border-line" style={{ backgroundColor: "#002366" }}>
                        <CardContent className="pt-6">
                          <p className="leading-relaxed text-white">{example}</p>
                        </CardContent>
                      </Card>
                    ))}
                  </div>
                  ...
                  <Accordion type="single" collapsible className="space-y-4">
                    {data.faqs.map((faq: any, idx: number) => (
                      <AccordionItem key={idx} value={`item-${idx}`} className="border border-line rounded-lg px-6">
                        ...
                        <AccordionContent className="leading-relaxed pt-2 text-foreground">
                          {faq.answer}
                        </AccordionContent>
                      </AccordionItem>
                    ))}
              ...
              <div className="lg:col-span-1">
                <Card className="border-line sticky top-24" style={{ backgroundColor: "#002366" }}>
                  <CardHeader>
                    <CardTitle className="text-xl font-serif text-white">Need Legal Counsel?</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <p className="text-sm leading-relaxed text-white">
                      Schedule a consultation to discuss your legal matter with our experienced attorneys.
                    </p>
                    <Button asChild className="w-full" style={{ backgroundColor: "#002366", color: "#ffffff", borderColor: "#ffffff", borderWidth: "1px" }}>
                      <Link to="/v4/contact">
                        Book Consultation
                        <ArrowRight className="ml-2 h-4 w-4" />
                      </Link>
                    </Button>
                    <div className="pt-4 border-t border-line" style={{ borderColor: "rgba(255, 255, 255, 0.2)" }}>
                      <p className="text-xs mb-2 text-white/80">Or reach us directly:</p>
                      <p className="text-sm font-medium text-white">+91 40 1234 5678</p>
                      <p className="text-sm text-white/90">contact@varunalaw.in</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
...
export default PracticeAreaDetailV4;
